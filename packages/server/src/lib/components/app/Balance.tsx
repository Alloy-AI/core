    ...
    <div className="text-sm text-muted-foreground flex justify-center items-center px-4 gap-x-4">
      <div className="flex justify-center items-center">
        <p>
          TMETIS:{" "}
          {formatEther(
            METISBalance?.data?.value ? METISBalance.data.value : 0n,
          )}
        </p>
        <Button
          variant="ghost"
          size="icon"
          onClick={() => METISBalance.refetch()}
        >
          <Icon
            name="RefreshCcw"
            className={cn(
      ...
      <div className="flex justify-center items-center">
        <p>
          USDT:{" "}
          {formatEther(USDTBalance?.data?.value ? USDTBalance.data.value : 0n)}
        </p>
        <Button
          variant="ghost"
          size="icon"
          onClick={() => USDTBalance.refetch()}
        >
          <Icon
            name="RefreshCcw"
            className={cn(
