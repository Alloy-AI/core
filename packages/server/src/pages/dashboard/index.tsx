...
import { PlusIcon, RobotIcon, PulseIcon, LightningIcon, FileTextIcon, TrendUpIcon } from "@phosphor-icons/react";
        ...
        <Layout>
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                        { label: "Total Agents", value: mockAgents.length.toString(), change: "+2 this week", icon: RobotIcon },
                        { label: "Active Sessions", value: activeSessions.toString(), change: "+12% vs last week", icon: PulseIcon },
                        { label: "Total Calls", value: totalCalls, change: "+150 this month", icon: TrendUpIcon },
                    ].map((stat, i) => (
                        <Card key={i} className="bg-background/40 backdrop-blur-md border-border">
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                {recentAgents.map((agent, i) => (
                                    <div key={i} className="flex items-center justify-between p-4 rounded-xl border border-border bg-muted/30 hover:bg-muted/50 transition-colors group cursor-pointer">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h3 className="font-medium text-foreground group-hover:text-primary transition-colors">{agent.name}</h3>
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className={`inline-block w-1.5 h-1.5 rounded-full ${agent.isActive ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]" : "bg-chart-2"}`} />
                                                    <span className="text-xs text-muted-foreground">{agent.isActive ? "Active" : "Idle"}</span>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="space-y-6">
                        <Card className="bg-gradient-to-br from-primary/20 via-background/40 to-background/40 border-primary/20 backdrop-blur-md">
                            ...
                            <CardContent className="grid grid-cols-2 gap-3">
                                <Button variant="outline" className="h-20 flex flex-col items-center justify-center gap-2 border-border hover:bg-muted/50 hover:text-foreground hover:border-primary/50 transition-all">
