...
export function registerA2A(args: { name: string, description: string, image: string | undefined }): AgentCard {
...
export async function callOtherAgent(agentUrl: string, method: string, params: any) {
    ...
    const card = await fetch(`${agentUrl}/.well-known/a2a.json`).then(r => r.json());
