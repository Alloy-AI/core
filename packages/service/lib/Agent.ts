import { IAgent, AgentDescriptor } from "../types/agent";
...
export class Agent implements IAgent {
    ...
    async generateResponse(args: { message: string; chatId?: string; }): Promise<string> {
        ...
        if (chatId) {
            try {
                ...
                console.warn(`Chat with ID ${chatId} not found, proceeding without chat context`);
            }
        }
    ...
    async getChatInformation(args: { chatId: string; }): Promise<AgentDescriptor> {
        ...
        let registration: AgentDescriptor['registration'] | null = null;
        if (!registration) {
    ...
    async addMCPServer(args: { name: string; transport: "stdio" | "websocket" | "custom"; connectionInfo: any; }): Promise<string> {
